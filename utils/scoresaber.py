...
def calculate_pp(acc, stars):
    return round(stars * pp_multiplier * interpolation.lerp(acc, curve),2)

def calculate_acc(target_pp, stars):
    tolerance = 0.0001

    max_possible = stars * (pp_multiplier * interpolation.lerp(100, curve))
    if target_pp > max_possible:
        return None
    ...
    while high - low > tolerance:
        mid = (low + high) / 2
        pp = stars * (pp_multiplier * interpolation.lerp(mid, curve))
